...
// Default / fallback Firebase config placeholders (can be updated by user)
...
export function initFirebase(configParams?: FirebaseConfigParams): { db: Firestore | null; isReal: boolean } {
...
function getLocalUserData(username: string): { matrix: ShelfMatrixMetadata | null; books: Record<string, BookRecord> } {
  try {
...
function saveLocalUserData(username: string, data: { matrix: ShelfMatrixMetadata | null; books: Record<string, BookRecord> }) {
  ...
  window.dispatchEvent(new CustomEvent('librislink_data_updated', { detail: { username } }));
}

// ---------------- USER AUTHENTICATION ----------------

export async function createAccount(username: string, passwordPlain: string): Promise<{ success: boolean; message: string }> {
  ...
  if (db) {
    try {
      ...
      return { success: true, message: 'Account created successfully in Firestore!' };
    } catch (err: any) {
      console.warn('Firestore error during signup, falling back to local storage:', err);
    }
  }
...
export async function loginUser(username: string, passwordPlain: string): Promise<{ success: boolean; message: string; user?: UserAccount }> {
  const cleanUsername = username.trim().toLowerCase();
  if (!cleanUsername) {
    return { success: false, message: 'Username is required.' };
  }
  ...
  if (db) {
    try {
      ...
      if (userSnap.exists()) {
        ...
        if (isValid) {
          return { success: true, message: 'Login successful via Firestore!', user: userData };
        } else {
          return { success: false, message: 'Invalid password.' };
        }
      }
    } catch (err) {
      console.warn('Firestore error during login, falling back to local storage:', err);
    }
  }
  ...
  if (!user) {
    ...
    if (cleanUsername === 'admin_demo' && passwordPlain === 'password123') {
      ...
      const demoUser: UserAccount = { username: 'admin_demo', passwordHash, createdAt: new Date().toISOString() };
      localUsers['admin_demo'] = demoUser;
      saveLocalUsers(localUsers);
      seedInitialSampleData('admin_demo');
      return { success: true, message: 'Welcome to LibrisLink Demo!', user: demoUser };
    }
...
export async function saveShelfMatrix(username: string, matrix: ShelfMatrixMetadata): Promise<void> {
  const colName = `user_${username}`;
...
export async function getShelfMatrix(username: string): Promise<ShelfMatrixMetadata | null> {
  const colName = `user_${username}`;
  if (db) {
    try {
...
export async function saveBooksBatch(username: string, books: BookRecord[]): Promise<void> {
  const colName = `user_${username}`;
  const local = getLocalUserData(username);
...
export async function saveSingleBook(username: string, book: BookRecord): Promise<void> {
  await saveBooksBatch(username, [book]);
}

export async function deleteSingleBook(username: string, bookId: string): Promise<void> {
  const colName = `user_${username}`;
  if (db) {
    try {
...
export function calculateStatus(qty: number, reservedBy: string | null): BookStatus {
  if (qty <= 0) return 'Out of Stock';
  if (reservedBy) return 'Reserved';
  return 'Available';
}

export async function decrementBookQty(username: string, bookId: string): Promise<void> {
  const colName = `user_${username}`;
  const local = getLocalUserData(username);
...
export async function zeroOutBookQty(username: string, bookId: string): Promise<void> {
  const colName = `user_${username}`;
  const local = getLocalUserData(username);
  const book = local.books[bookId];
...
export async function reserveOrQueueBook(username: string, bookId: string, appUserId: string): Promise<{ success: boolean; actionTaken: string }> {
  const colName = `user_${username}`;
  const local = getLocalUserData(username);
  const book = local.books[bookId];
...
export async function releaseReservation(username: string, bookId: string): Promise<void> {
  const colName = `user_${username}`;
  const local = getLocalUserData(username);
  const book = local.books[bookId];
...
export function subscribeToInventory(
  username: string,
  onData: (data: { matrix: ShelfMatrixMetadata | null; books: BookRecord[] }) => void,
